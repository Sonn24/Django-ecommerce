...
def calculator(request):
    ...
    return render(request, "calculator.html", {'c': result})
...
def subscribe(request):
    if request.method == 'POST':
        email = request.POST.get('email')
        # Save to DB or send to email service here
...
def add_to_cart(request, slug):
    ...
    if size:
        try:
            selected_size = ItemSize.objects.get(item=item, size=size)
            selected_price = selected_size.price
        except ItemSize.DoesNotExist:
            selected_size = None

    # Create or get an active order
    order, created = Order.objects.get_or_create(user=request.user, ordered=False)

    # Check if this exact variation (size + color) exists in cart
...
def pay(request):
    ...
    if request.method == "POST":
        ...
        for item in order.items.all():
            message += f"- {item.quantity} x {item.item.title} (Size: {item.size}, Color: {item.color}) @ {item.get_final_price}\n"

        email = EmailMessage(subject, message, settings.DEFAULT_FROM_EMAIL, ["raymediagraphix@gmail.com"])
        email.send()

        # --- Email to customer ---
        customer_subject = "Your order has been received!"
        customer_message = f"""
        Hi {customer_name},
        ...
        """
        for item in order.items.all():
            customer_message += f"- {item.quantity} x {item.item.title} (Size: {item.size}, Color: {item.color}) @ {item.get_final_price}\n"

        customer_message += f"\nTotal: KES {amount}\n\nWe will contact you shortly for delivery."

        customer_email = EmailMessage(customer_subject, customer_message, settings.DEFAULT_FROM_EMAIL, [email_address])
        customer_email.send()
        ...
        # Create a new empty order (so order-summary has something to show)
        Order.objects.get_or_create(user=request.user, ordered=False)

        messages.success(request, "Order placed successfully! Your cart is now empty.")
        return redirect("core:order-summary")
